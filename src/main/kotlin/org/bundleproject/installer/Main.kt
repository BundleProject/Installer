...
suspend fun installOfficial(path: File, mcversion: String) {
    ...
    versionJson.addLibrary(
        VersionLibrary(
            name = "org.bundleproject:launchwrapper:$latest",
            url = "https://jitpack.io/org/bundleproject/launchwrapper/$latest/launchwrapper-$latest.jar",
            path = "org/bundleproject/launchwrapper/$latest/launchwrapper-$latest.jar",
            sha1 = http.get<String>("https://jitpack.io/org/bundleproject/launchwrapper/$latest/launchwrapper-$latest.jar.sha1"),
            size = http.head<HttpResponse>("https://jitpack.io/org/bundleproject/launchwrapper/$latest/launchwrapper-$latest.jar").contentLength()!!.toInt()
        )
    )
